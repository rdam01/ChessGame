...
namespace ChessGame
{
    public class PolyglotBook
    {
        ...
        private readonly Dictionary<ulong, List<BookMove>> _bookMoves;
        private readonly Random _random;
        
        public PolyglotBook(string bookPath)
        {
            _bookPath = bookPath;
            _bookMoves = new Dictionary<ulong, List<BookMove>>();
            _random = new Random();
            LoadBook();
        }
        
        // Get a move from the opening book
        public MoveResultStruct GetBookMove(IBoard board, Color sideToMove)
        {
            ulong key = GeneratePolyglotKey(board, sideToMove);
            
            if (_bookMoves.TryGetValue(key, out List<BookMove> moves))
            {
                // Select move based on weights
                BookMove selectedMove = SelectWeightedMove(moves);
                
                // Convert to game move
                return ConvertBookMoveToGameMove(selectedMove, board, sideToMove);
            }
            
            // Return invalid move if position not in book
        ...
        private void LoadBook()
        {
            try
            {
                using (FileStream fs = new FileStream(_bookPath, FileMode.Open, FileAccess.Read))
                using (BinaryReader reader = new BinaryReader(fs))
                {
                    // Each entry is 16 bytes
                    int entryCount = (int)(fs.Length / 16);
                    
                    for (int i = 0; i < entryCount; i++)
                    {
                        // Read key (8 bytes) - Convert from Big Endian to Little Endian
                        byte[] keyBytes = reader.ReadBytes(8);
                        Array.Reverse(keyBytes);
                        ulong key = BitConverter.ToUInt64(keyBytes, 0);
                        
                        // Read move (2 bytes) - Convert from Big Endian to Little Endian
                        byte[] moveBytes = reader.ReadBytes(2);
                        Array.Reverse(moveBytes);
                        ushort moveData = BitConverter.ToUInt16(moveBytes, 0);
                        
                        // Read weight (2 bytes) - Convert from Big Endian to Little Endian
                        byte[] weightBytes = reader.ReadBytes(2);
                        Array.Reverse(weightBytes);
                        ushort weight = BitConverter.ToUInt16(weightBytes, 0);
                        
                        // Read learn value (4 bytes, usually ignored) - Convert from Big Endian to Little Endian
                        byte[] learnBytes = reader.ReadBytes(4);
                        Array.Reverse(learnBytes);
                        uint learn = BitConverter.ToUInt32(learnBytes, 0);
                        
                        // Decode move according to Polyglot format
                        // Bits 0-2: to file
                        // Bits 3-5: to row
                        // Bits 6-8: from file
                        // Bits 9-11: from row
                        ...
                        int toRow = (moveData >> 3) & 0x7;
                        int fromFile = (moveData >> 6) & 0x7;
                        int fromRow = (moveData >> 9) & 0x7;
                        int promotion = (moveData >> 12) & 0x7;
                        
                        // Calculate square indices (0-63)
                        int fromSquare = fromRow * 8 + fromFile;
                        int toSquare = toRow * 8 + toFile;
                        
                        // Ignore moves that are "0" (a1a1)
                        if (moveData == 0 || (fromFile == 0 && fromRow == 0 && toFile == 0 && toRow == 0))
                        {
                            continue;
                        }
                        
                        // Create book move
                        BookMove move = new BookMove
                        {
                        ...
                        // Add to dictionary
                        if (!_bookMoves.ContainsKey(key))
                        {
                            _bookMoves[key] = new List<BookMove>();
                        }
                        
                        _bookMoves[key].Add(move);
                    }
                    
                    Console.WriteLine($"Loaded {_bookMoves.Count} positions from Polyglot book");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error loading Polyglot book: {ex.Message}");
                // Initialize empty dictionary if book can't be loaded
                _bookMoves.Clear();
            }
        }
        
        private BookMove SelectWeightedMove(List<BookMove> moves)
        {
        ...
        private MoveResultStruct ConvertBookMoveToGameMove(BookMove bookMove, IBoard board, Color sideToMove)
        {
            try
            {
                // In Polyglot, squares are numbered from 0 to 63, with a1=0, b1=1, ..., h8=63
                // Convert square indices to file/rank
                int fromFile = bookMove.FromSquare % 8;
                int fromRank = bookMove.FromSquare / 8;
                int toFile = bookMove.ToSquare % 8;
                int toRank = bookMove.ToSquare / 8;
                
                // Handle promotion piece
                // 0: none, 1: knight, 2: bishop, 3: rook, 4: queen
                PieceType? promotionPiece = bookMove.Promotion switch
                {
                    1 => PieceType.Knight,
                    2 => PieceType.Bishop,
                    3 => PieceType.Rook,
                ...
                // Debug output
                Console.WriteLine($"Book move: from {(Column)fromFile},{fromRank} to {(Column)toFile},{toRank}");
                
                // Get squares - in our engine, rank 0 is the bottom row (white's first rank)
                Square fromSquare = board.GetSquare((Column)fromFile, fromRank);
                Square toSquare = board.GetSquare((Column)toFile, toRank);
                
                if (fromSquare == null || toSquare == null)
                {
                    Console.WriteLine($"Invalid squares: from {(Column)fromFile},{fromRank} to {(Column)toFile},{toRank}");
                    return new MoveResultStruct { MoveResult = MoveResult.Invalid };
                }
                
                if (fromSquare.Piece == null)
                {
                    Console.WriteLine($"No piece at from square: {(Column)fromFile},{fromRank}");
                    return new MoveResultStruct { MoveResult = MoveResult.Invalid };
                }
                
                // Debug output
                Console.WriteLine($"From square: {fromSquare.Column},{fromSquare.Row}, To square: {toSquare.Column},{toSquare.Row}");
                Console.WriteLine($"Piece at from square: {fromSquare.Piece.Type}, Color: {fromSquare.Piece.Color}");
                
                // Check if the piece at the from square belongs to the current player
                if (fromSquare.Piece.Color != sideToMove)
                {
                    Console.WriteLine($"Piece at from square belongs to {fromSquare.Piece.Color}, but current player is {sideToMove}");
                    return new MoveResultStruct { MoveResult = MoveResult.Invalid };
                }

                // Make the move
                // Note: The current implementation doesn't directly support passing promotion pieces
                // to the Move method. If promotion handling is needed, the Board implementation
                // would need to be modified to detect promotion moves based on piece type and position.                
                
                MoveResultStruct result = new MoveResultStruct();
                
                result.From = fromSquare;
                result.To = toSquare;
                result.MoveResult = MoveResult.OK; // validity will be checked later on

                /*board.Move(sideToMove, fromSquare, toSquare);                */

                // If this is a promotion move, log it for debugging
                if (promotionPiece.HasValue)
                {
                    Console.WriteLine($"Promotion move detected: {promotionPiece.Value}");
                    // The board implementation should handle promotions internally based on
                    // the piece type and the destination square being on the 8th/1st rank
                }
                
                // Debug output
                Console.WriteLine($"Move result: {result.MoveResult}");
                
                return result;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error converting book move: {ex.Message}");
                Console.WriteLine($"Stack trace: {ex.StackTrace}");
                return new MoveResultStruct { MoveResult = MoveResult.Invalid };
            }
        }
        
        private ulong GeneratePolyglotKey(IBoard board, Color sideToMove)
        {
            ulong key = 0;
            ...
            for (Column col = Column.a; col <= Column.h; col++)
            {
                for (int row = 0; row <= 7; row++)
                {
                    ...
                    {
                        int piece = GetPolyglotPieceIndex(square.Piece.Type, square.Piece.Color);
                        int squareIndex = (int)col + row * 8;
                        key ^= PolyglotRandoms[64 * piece + squareIndex];
                    }
                }
            }
            
            // Castling rights
            CastlingRight whiteCastling = board.GetCastlingRight(Color.White);
            CastlingRight blackCastling = board.GetCastlingRight(Color.Black);

            if (whiteCastling == CastlingRight.Both)
            {
                key ^= PolyglotRandoms[768]; // White kingside
                key ^= PolyglotRandoms[769]; // White queenside
            }
            else
            {
                if (whiteCastling == CastlingRight.Short)
                    key ^= PolyglotRandoms[768]; // White kingside
                else if (whiteCastling == CastlingRight.Long)
                    key ^= PolyglotRandoms[769]; // White queenside
            }

            if (blackCastling == CastlingRight.Both)
            {
                key ^= PolyglotRandoms[770]; // Black kingside
                key ^= PolyglotRandoms[771]; // Black queenside
            }
            else
            {
                if (blackCastling == CastlingRight.Short)
                    key ^= PolyglotRandoms[770]; // Black kingside
                else if (blackCastling == CastlingRight.Both || blackCastling == CastlingRight.Long)
                    key ^= PolyglotRandoms[771]; // Black queenside
            }

            /*
            // En passant square // TODO: fix
            if (board.EnPassantSquare != null)
            {
                int file = (int)board.EnPassantSquare.Column;
                key ^= PolyglotRandoms[772 + file];
            }
            */
        ...
        // Convert piece type and color to Polyglot piece index
        private int GetPolyglotPieceIndex(PieceType type, Color color)
        {
            /*
            // Polyglot uses: 0=white pawn, 1=white knight, 2=white bishop, 3=white rook,
            // 4=white queen, 5=white king, 6=black pawn, 7=black knight, etc.
            int pieceIndex = type switch
            {
                PieceType.Pawn => 0,
                PieceType.Knight => 1,
                PieceType.Bishop => 2,
        ...
        // These are the standard values used by all Polyglot implementations
        private static readonly ulong[] PolyglotRandoms = {
   ...
   0xF8D626AAAF278509
    ...
    // Class to represent a move in the book
